...
### Step 0.1 — Add `expiresAt` to the token ledger
**Goal:** Lay the groundwork for "no rollover" / "1-year top-up expiry" without touching balance logic yet.
**New files:** `prisma/migrations/<timestamp>_add_token_expiry/migration.sql` (generated by `prisma migrate dev`)
**Edited files:** `prisma/schema.prisma` (add `expiresAt DateTime?` to `TokenTransaction`)
**New packages:** none
**New env vars:** none
**Test:** Run `npx prisma migrate dev`, confirm the column exists via `npx prisma studio`. Existing token flows should be unaffected since the field defaults to `null`.

### Step 0.2 — Filter expired credits out of the balance calculation
**Goal:** Make `getUserTokenBalance` respect `expiresAt` (Option A from the PRD).
**New files:** none
**Edited files:** `src/lib/tokens.ts`
**Test:** In Prisma Studio, manually set an old credit transaction's `expiresAt` to a past date, call `GET /api/tokens/balance`, confirm that transaction's amount is excluded from the returned balance. Then clear it back and confirm the balance is restored.

### Step 0.3 — Switch NextAuth to database session strategy
**Goal:** Fix the underlying cause of the fake mock-session bug (PRD §6.9.3) before building real session UI on top of it.
**New files:** none
**Edited files:** `src/auth.ts` (change `session: { strategy: 'jwt' }` → `session: { strategy: 'database' }`), `src/proxy.ts` (verify the cookie-session fallback path still resolves correctly against a DB session)
**New packages:** none
**New env vars:** none
**Test:** Log in via the web UI, confirm a row now appears in the `Session` table in Prisma Studio. Log out, confirm the row is removed/expired. Confirm the mobile Bearer-token login flow (`/api/auth/login`) still works unaffected (it doesn't use this session strategy at all).

### Step 0.4 — Remove the synthetic mock-session seeding
...
### Step 0.5 — Install and configure i18next
...
### Step 0.6 — Pilot i18n extraction on one small surface
...
### Step 0.7 — Add `NotificationPreference` model
...
### Step 0.8 — Build `/api/settings/notifications` CRUD routes
...
### Step 0.9 — Wire the notifications settings UI to the real API
...
### Step 0.10 — Build a generic `notify()` helper and refactor the verification email through it
...
### Step 0.11 — Add in-app `Notification` model and list/read API
...
> Depends on Phase 0 steps 0.1–0.2 (token expiry) and 0.7–0.11 (notifications, for payment-confirmation notices) being merged.
...
**New files:** `src/app/api/payments/stripe/webhook/route.ts` (verifies `stripe-signature` header via `stripe.webhooks.constructEvent`, and on `checkout.session.completed` creates/updates a `Subscription` row, credits tokens via `TokenTransaction` with the correct `expiresAt`, and records a `PaymentTransaction`)
**Edited files:** none
...
**New files:** `src/app/api/payments/paystack/webhook/route.ts` (verifies the `x-paystack-signature` header using HMAC-SHA512 with `PAYSTACK_SECRET_KEY` via Node's built-in `crypto`, then on `charge.success` performs the same credit/Subscription/PaymentTransaction logic as Step 1.7)
**Edited files:** none
