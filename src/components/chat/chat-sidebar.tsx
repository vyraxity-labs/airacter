...
export function ChatSidebar({ chats, activeChatId, onDeleteChat, onRenameChat }: ChatSidebarProps) {
  ...
  const handleSaveRename = async (e: React.FormEvent | React.MouseEvent, chatId: string) => {
    if (e.type === "submit") {
      e.preventDefault();
    }
    e.stopPropagation();
      ...
      <div className="flex-grow overflow-y-auto custom-scrollbar p-2 space-y-4">
        {chatGroups.length === 0 ? (
          <div className="py-12 px-4 text-center">
            <MessageSquare className="mx-auto text-outline/40 mb-3 stroke-[1.5px]" size={32} />
            <p className="text-xs font-semibold text-on-surface-variant">No active conversations</p>
...
function ChatSidebarAvatar({ character }: { character: Character }) {
  ...
  const fallbackText = character.avatarType === "emoji" 
