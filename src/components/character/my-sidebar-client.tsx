...
interface MySidebarClientProps {
  createdCharacters: Character[]
}

const MySidebarClient = ({ createdCharacters }: MySidebarClientProps) => {
  ...
  return (
    <div className='flex flex-col h-full bg-surface-low select-none font-sans'>
      ...
      <div className='px-4 pt-6 mb-6'>
        ...
        >
          <PlusCircle size={18} />
          {t('sidebar.create_character')}
        </Link>
      </div>

      {/* Characters List */}
      <div className='grow overflow-y-auto px-2 space-y-1 custom-scrollbar pb-6'>
        <div className='px-4 py-2 text-[10px] text-outline font-bold uppercase tracking-widest'>
          {t('sidebar.my_characters')}
        </div>
            ...
            return (
              ...
              >
                ...
                <div className='flex-1 overflow-hidden'>
                  <p
                    className={`text-sm font-bold truncate ${isFirst ? 'text-primary' : 'text-on-surface'}`}
                  >
                    {char.name}
                  </p>
...
export default MySidebarClient
