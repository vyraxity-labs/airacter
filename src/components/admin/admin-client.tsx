...
export function AdminClient() {
  ...
  const handleConfirmReject = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}))
        throw new Error(data.error || 'Rejection request failed')
      }
  ...
  const handleDismissReport = async (reportId: string, characterId: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}))
        throw new Error(data.error || 'Dismiss report request failed')
      }
  ...
  const handleToggleFeature = async (
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}))
        throw new Error(data.error || 'Feature toggle request failed')
      }

      setPendingChars((prev) =>
        prev.map((c) =>
          c.id === characterId ? { ...c, isFeatured: !currentFeatured } : c,
        ),
      )
    } catch (err: any) {
        ...
        {loading ? (
                ...
                {pendingChars.map((char) => (
                  ...
                  >
                    ...
                    <div className='flex gap-2 pt-2'>
                      ...
                      <button
                        onClick={() =>
                          handleToggleFeature(char.id, char.isFeatured)
                        }
                        disabled={processingId !== null}
                        className={cn(
            ...
            {reports.length === 0 ? (
              ...
              <div className='space-y-4'>
                {reports.map((report) => (
                  <div
                    key={report.id}
                    className='glass-panel rounded-3xl p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-6'
                  >
                      ...
                      <div className='space-y-2'>
                        <div className='flex flex-wrap items-center gap-2'>
                          ...
                          <span className='text-[10px] text-on-surface-variant'>
                            Target Character:{' '}
                            <strong>{report.character.name}</strong>
                          </span>
                        ...
                        <div className='text-[10px] text-on-surface-variant'>
                          Reported by: <strong>{report.user.email}</strong> on{' '}
                    ...
                    <div className='flex gap-2 w-full md:w-auto'>
                      <button
                        onClick={() =>
                          handleDismissReport(report.id, report.characterId)
                        }
                        disabled={processingId !== null}
                      ...
                      <button
                        onClick={() =>
                          handleOpenReject(report.characterId, report.id, true)
                        }
                        disabled={processingId !== null}
          ...
          <section className='glass-panel rounded-3xl p-8 max-w-xl'>
            ...
            <p className='text-xs text-on-surface-variant leading-relaxed mb-6'>
              Adjust user available token balance by submitting an audit log.
              Positive values credit tokens; negative values debit tokens.
            </p>
      ...
      {showRejectModal && (
        <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm'>
          <div className='w-full max-w-md p-6 rounded-2xl glass-panel border border-border bg-surface shadow-2xl relative'>
            ...
            <p className='text-xs text-on-surface-variant leading-relaxed mb-6'>
              This will set the character to <strong>private</strong> and log a
              24-hour resubmission block. The creator will see the reason below.
            </p>
