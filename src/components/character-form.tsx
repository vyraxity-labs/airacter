...
const TONE_OPTIONS = ["Casual", "Formal", "Humorous", "Stoic", "Empathetic", "Dramatic"];
const CATEGORIES = ["education", "productivity", "entertainment", "wellness", "creative", "technical", "fun", "other"];
...
export function CharacterForm({ initialData }: CharacterFormProps) {
  ...
  const [avatarType, setAvatarType] = useState<"emoji" | "initials" | "image">(initialData?.avatarType || "emoji");
  const [avatarValue, setAvatarValue] = useState(initialData?.avatarValue || "🎭");
  const [avatarColor, setAvatarColor] = useState(initialData?.avatarColor || "#ffafd3");
  ...
  const [sandboxMessages, setSandboxMessages] = useState<Array<{ role: "user" | "assistant"; content: string }>>([
    {
      role: "assistant",
      content: "Hey there! I'm your new character. Start typing in the form to shape how I respond to you here!",
    },
  ]);
  ...
  const handleAvatarTypeChange = (type: "emoji" | "initials" | "image") => {
    ...
    setUploadError("");
    if (type === "emoji") {
      setAvatarValue(initialData?.avatarType === "emoji" ? initialData.avatarValue : "🎭");
      setAvatarColor(initialData?.avatarColor || "#ffafd3");
    } else if (type === "initials") {
      setAvatarValue(initialData?.avatarType === "initials" ? initialData.avatarValue : getInitials(name));
      setAvatarColor(initialData?.avatarColor || "#d2bbff");
    } else {
      setAvatarValue(initialData?.avatarType === "image" ? initialData.avatarValue : "");
    }
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.error(err);
      setUploadError(err.message || "Failed to upload image. Ensure Cloudinary credentials are set.");
    } finally {
  ...
  const handleSandboxSend = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (lowerMsg.includes("who are you") || lowerMsg.includes("your name") || lowerMsg.includes("personality")) {
        reply = `I am ${charName}, ${charDesc}. My directives are shaped by my tones: ${activeTones}. Nice to meet you!`;
      } else if (lowerMsg.includes("prompt") || lowerMsg.includes("instruction") || lowerMsg.includes("system")) {
  ...
  const validateForm = () => {
    ...
    if (name.trim().length < 3) errors.name = "Character name must be at least 3 characters.";
    if (name.trim().length > 60) errors.name = "Character name cannot exceed 60 characters.";
    if (description.trim().length === 0) errors.description = "Short description is required.";
    if (description.trim().length > 200) errors.description = "Description cannot exceed 200 characters.";
    if (systemPrompt.trim().length < 20) errors.systemPrompt = "System prompt must be at least 20 characters.";
    if (systemPrompt.trim().length > 2000) errors.systemPrompt = "System prompt cannot exceed 2000 characters.";
    ...
    if (avatarType === "image" && !val) {
      ...
      if (!val) {
        ...
        if (isEmojiString(val)) {
          ...
          if (charCount > 5) {
            errors.avatarValue = "Maximum of 5 characters allowed for normal text emoji.";
          }
        }
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        const url = initialData?.id ? `/api/characters/${initialData.id}` : "/api/characters";
        ...
        console.error(err);
        setErrorMsg(err.message || "Failed to save character. Please try again.");
      }
    });
  };
  ...
  return (
    <div className="flex-grow flex h-full overflow-hidden relative">
      {/* Form Area (Left) */}
      <div className="flex-grow overflow-y-auto custom-scrollbar px-6 md:px-12 py-10">
        ...
        <form onSubmit={handleSubmit} className="max-w-3xl space-y-10">
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3 border-b border-border/10 pb-2">
              ...
              <h2 className="text-lg font-bold text-on-surface">Identity Blueprint</h2>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-[140px_1fr] gap-6 md:gap-8 items-start">
              {/* Avatar Selector Panel */}
              <div className="flex flex-col items-center gap-3">
                <div className="w-[120px] h-[120px] rounded-full relative overflow-hidden shadow-lg p-0.5 bg-surface-container-high border border-border/20">
                  {renderAvatarPreview()}
                </div>
                <span className="text-[10px] uppercase font-extrabold tracking-widest text-outline">Avatar View</span>
              </div>
                      ...
                      {POPULAR_EMOJIS.map((emoji) => (
                        <button
                          ...
                          className={cn(
                            ...
                            avatarValue === emoji && "border-primary bg-primary/15"
                          )}
                        >
                          {emoji}
                        </button>
                    ...
                    {COLOR_PALETTE.map((color) => (
                      <button
                        ...
                        className={cn(
                          ...
                          avatarColor === color.value ? "scale-105" : "border-background"
                        )}
                        style={{ backgroundColor: color.value, borderColor: avatarColor === color.value ? "#ffffff" : "transparent" }}
                        title={color.label}
                      >
                        {avatarColor === color.value && (
                          <Check size={14} className="text-white drop-shadow-md stroke-[3px]" />
                        )}
                      </button>
                    ))}
                  </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3 border-b border-border/10 pb-2">
              ...
              <h2 className="text-lg font-bold text-on-surface">Directive instructions (System Prompt)</h2>
            </div>
            ...
            <p className="text-xs text-outline leading-normal font-sans">
              This prompt acts as the character's subconscious rules. Specify their identity, speaking style, forbidden topics, memory rules, and backstories.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              ...
              >
                {CATEGORIES.map((cat) => (
                  <option key={cat} value={cat} className="bg-surface text-on-surface capitalize">
                    {cat}
                  </option>
                ))}
              </select>
            ...
            <div className="flex items-center justify-between p-4 bg-surface-container-low rounded-xl border border-border/10">
              <div className="max-w-[80%]">
                ...
                <p className="text-xs text-on-surface-variant mt-0.5 leading-normal">
                  Toggle on to list this character in the public Explore tab. Off keeps it hidden in your library only.
                </p>
              </div>
      ...
      <aside className="hidden lg:flex flex-col w-[380px] h-full bg-surface-container-low border-l border-border/40 select-none font-sans">
        {/* Header */}
        <div className="h-16 flex items-center justify-between px-6 border-b border-border/40">
          <span className="font-bold text-sm text-primary tracking-tight">Live Sandbox Preview</span>
              ...
              >
                ...
                <div
                  className={cn(
                    ...
                    isUser
                      ? "bg-gradient-to-br from-primary to-secondary text-white border-transparent rounded-br-none"
        ...
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="w-full max-w-md bg-surface-container rounded-2xl p-6 border border-error/20 shadow-2xl relative">
            ...
            <p className="text-sm text-on-surface-variant mb-6 leading-relaxed">
              This action cannot be undone. All database records (including saves and chat histories) associated with <strong>{name}</strong> will be permanently wiped.
            </p>
            
            <div className="space-y-4">
              <label className="block text-xs font-bold text-on-surface-variant uppercase tracking-wider">
                Type character name <span className="text-error">"{name}"</span> to confirm:
              </label>
              <input
                type="text"
