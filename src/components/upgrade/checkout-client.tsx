  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isMounted.current) {
        router.push(
          `/upgrade/success?packId=${pack.id}&tx=${data.transactionId}`,
        )
      }
    } catch (err: any) {
      if (isMounted.current) {
  ...
  return (
    <div className='flex-grow min-h-screen overflow-y-auto custom-scrollbar select-none bg-background pb-24'>
      <div className='max-w-5xl mx-auto py-10 px-8'>
        {/* BACK ACTION */}
        <Link
          href={isProcessing ? '#' : '/upgrade'}
          onClick={(e) => isProcessing && e.preventDefault()}
          className={cn(
            'inline-flex items-center gap-1.5 text-xs font-bold mb-8 transition-colors',
            isProcessing
