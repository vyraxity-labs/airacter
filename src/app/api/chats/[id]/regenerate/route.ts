  ...
  try {
    ...
    if (balance <= 0) {
      return NextResponse.json(
        { error: "Insufficient token balance. Please purchase more tokens to continue chatting." },
    ...
    if (lastUserIndex === -1) {
      return NextResponse.json({ error: "No user message to regenerate a response for" }, { status: 400 });
    }

    const lastUserMessage = dbMessages[lastUserIndex];

    // 4. Delete all messages after this user message
    ...
    const history: ChatMessage[] = dbMessages.slice(0, lastUserIndex).map((m) => ({
    ...
    const customStream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          const historyWithUser = [...history, { role: "user" as const, content: lastUserMessage.content }];
