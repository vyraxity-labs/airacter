...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "approve") {
      ...
      const rejectReason = reason?.trim() || "Does not meet community standards.";
      await db.$transaction([
        db.character.update({
          where: { id: characterId },
      ...
      if (!reportId) {
        return NextResponse.json({ error: "reportId is required for dismiss_report action" }, { status: 400 });
      }
      ...
      if (!reportId) {
        return NextResponse.json({ error: "reportId is required for resolve_report action" }, { status: 400 });
      }
      
      const rejectReason = reason?.trim() || "Report resolved. Content restricted.";
