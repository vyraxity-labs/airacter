...
**Revised decision (supersedes the v1.0 "just filter expired credits" approach — a real accounting flaw was found in that design):** a naive `SUM(unexpired credits) − SUM(all debits)` balance query is wrong. Because debits are subtracted from the *current* pool of unexpired credits rather than from the specific grant they were actually spent against, a user who spends less than their free welcome tokens in a month can still lose *paid* top-up tokens once the welcome batch expires — the formula re-applies old spending against the wrong pool retroactively. Concretely: 10,000 welcome tokens (30-day expiry) + 5,000 purchased top-up tokens (1-year expiry), 8,000 tokens spent in month one. After the welcome batch expires, `SUM(unexpired credits) = 5,000` and `SUM(all debits) = 8,000`, so the naive formula reports a negative/zero balance — the 5,000 paid tokens the user never actually spent are effectively erased.

**Decision: introduce a `TokenGrant` table and make FIFO (earliest-expiry-first) consumption the canonical, atomic spend mechanism — not just a balance-calculation trick.**
...
- Every credit (welcome bonus, subscription refresh, top-up purchase, admin adjustment) inserts a new `TokenGrant` with `remaining = amount` and the correct `expiresAt` (`null` for non-expiring admin grants, subscription-period-end for subscription tokens, +1 year for top-ups).
- Every debit (a chat message) runs inside a single database transaction that locks the user's unexpired, non-exhausted grants ordered `expiresAt ASC NULLS LAST` (soonest-expiring first — this is also simply the right order for the user, not only an accounting nicety), verifies total available `remaining` covers the debit, and decrements across grants in that order. Row-level locking (e.g. `SELECT ... FOR UPDATE` inside a Prisma interactive transaction) is required here — without it, two concurrent chat requests could both read the same "sufficient balance" snapshot and both succeed, overspending the user's real balance. This atomicity requirement is not optional at any usage scale, since concurrent requests from the same user (e.g. two open tabs, or a retry) are routine, not an edge case.
- Balance becomes a trivial, cheap query: `SUM(remaining) WHERE remaining > 0 AND (expiresAt IS NULL OR expiresAt > now())`. No replay of history, no per-request recomputation cost as transaction history grows.
- **`TokenTransaction` is retained unchanged as the human-readable audit trail** (what the billing/history UI reads) — `TokenGrant` is what actually governs balance and expiry, `TokenTransaction` stays the ledger of record for "what happened and when." Every credit/debit operation writes to both.

This is a small but real scope increase over the v1.0 plan (a new model and a rewritten spend path, versus one nullable column and one query filter) — worth it because the simpler version has a bug that directly costs paying users their purchased tokens.
...
**This migration is not a one-line config flip; it changes what the session cookie *is*.** Under `jwt` strategy the cookie is a self-contained, signed JWT — `proxy.ts` can decode it locally via `getToken()` with no database access, which is why `proxy.ts` today builds a second, adapter-less `NextAuth(authConfig)` instance (originally to stay edge-runtime-safe). Under `database` strategy the cookie becomes an opaque random session token that only means something when looked up against the `Session` table — `getToken()` cannot decode it, and the adapter-less instance has no way to query the database. Both the page-protection path (the `nextAuthMiddleware(request)` call at the bottom of `proxy.ts`) and the API-route header-forwarding fallback break at the same time, for the same reason, not just the fallback in isolation.

Three things need to happen together, not just the strategy flip:
1. **Audit the JWT-specific callbacks in `auth.config.ts`.** The `jwt({ token, user })` callback simply stops being invoked under `database` strategy — harmless to leave, but dead code worth removing for clarity. The `session({ session, token })` callback, however, is **wrong** under `database` strategy: NextAuth calls it with `{ session, user }` instead of `{ session, token }` (`user` is the full database user record from the adapter). Left as-is, `session.user.role` silently becomes `undefined` on every request, since it's currently read off `token.role`, and `token` will never be populated. This must be changed to read off `user.role` before the strategy switch goes live, or every authorization check downstream of it degrades silently rather than failing loudly.
2. **Determine whether `proxy.ts` can resolve sessions through the canonical, adapter-backed NextAuth instance (`auth` exported from `src/auth.ts`) instead of maintaining a second, adapter-less one.** The original reason for the second instance was edge-runtime safety, and Next.js 16 runs `proxy.ts` on the Node.js runtime by default (per `AGENTS.md`'s warning that this isn't the Next.js most training data reflects) — the constraint that motivated the split may no longer apply. If the canonical instance works directly in `proxy.ts`, that's the better outcome: one source of truth for session resolution instead of two mechanisms that can drift out of sync, and no new lookup code to write or maintain.
3. **Only if that genuinely doesn't work** (e.g. a real, confirmed constraint on using the adapter-backed instance inside `proxy.ts`'s request lifecycle), fall back to a direct database lookup — but isolate it behind a single helper (e.g. `resolveSessionFromCookie()`) used identically by both the page-protection branch and the API-header-forwarding branch, rather than duplicating Prisma session-lookup logic inline in two places in `proxy.ts`. Either way, the **mobile Bearer-JWT flow** (`lib/jwt.ts`, `/api/auth/login`, the `Authorization: Bearer` branch in `proxy.ts`) is untouched by any of this — it doesn't use NextAuth sessions at all and has no dependency on this change.
